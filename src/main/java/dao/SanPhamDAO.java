...
public class SanPhamDAO {
    ...
    public boolean update(SanPham sp) {
        ...
        if (check == null) {
            throw new RuntimeException("Sản phẩm chưa tồn tại!");
        }
    ...
    //Thêm
  public boolean tangTonKho(String maSP, int soLuong) {
    String sql = "UPDATE sanpham SET "
            + "tonkho = IFNULL(tonkho, 0) + ?, "
            + "trangthai = CASE WHEN IFNULL(tonkho,0) + ? > 0 THEN 'Còn hàng' ELSE 'HẾT HÀNG' END "
            + "WHERE maSP = ?";
