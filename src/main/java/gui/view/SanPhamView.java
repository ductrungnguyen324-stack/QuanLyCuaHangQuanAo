...
public class SanPhamView extends JFrame {
    ...
    private JPanel buildHeader() {
        ...
        lblTongSP  = makeChip("Tổng: --",     ACCENT);
        lblConHang = makeChip("Còn hàng: --", GREEN);
        lblSapHet  = makeChip("Hết hàng: --", RED);

        JPanel chips = new JPanel(new FlowLayout(FlowLayout.LEFT, 6, 0));
        chips.setOpaque(false);
        chips.add(lblTongSP);
        chips.add(lblConHang);
        chips.add(lblSapHet);
        ...
        btnThem = makeButton("+ Thêm sản phẩm", ACCENT, Color.WHITE);
    ...
    private JScrollPane buildTable() {
        ...
        int[] widths = {80, 200, 80, 120, 70, 80, 110, 70, 110, 120};
        for (int i = 0; i < widths.length; i++)
            table.getColumnModel().getColumn(i).setPreferredWidth(widths[i]);

        // Renderer Trạng thái (col 8)
        ...
        // Renderer Tồn kho (col 7)
        table.getColumnModel().getColumn(7).setCellRenderer(
                (t, val, sel, foc, row, col) -> {
                    JLabel l = new JLabel(String.valueOf(val));
                    l.setHorizontalAlignment(SwingConstants.CENTER);
                    l.setOpaque(true);
                    l.setFont(new Font("Dialog", Font.BOLD, 12));
                    l.setBackground(sel ? ROW_SEL : row % 2 == 0 ? SURFACE : ROW_ODD);
                    try {
                        int tk = Integer.parseInt(String.valueOf(val));
                        l.setForeground(tk == 0 ? RED : tk <= 5 ? YELLOW : GREEN);
                    } catch (Exception ignored) { l.setForeground(TEXT1); }
                    return l;
                }
        );
    ...
    public void renderDanhSach(List<SanPham> list) {
        ...
        for (SanPham sp : list) {
            tableModel.addRow(new Object[]{
                    ...
                    sp.getKichco()     != null ? sp.getKichco()     : "-",
    ...
    public void updateStats() {
        ...
        lblConHang.setText("Còn hàng: " + conHang);
        lblSapHet.setText("Hết hàng: "  + hetHang);
    }
    ...
    public JComboBox<String> getCbLoai()      { return cbLoai;      }
