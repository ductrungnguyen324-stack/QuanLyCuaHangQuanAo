...
/**
 * ThemNhanVienDialog.java
 * Package : gui.view.dialogs
 * Chuc nang: Dialog THEM MOI nhan vien — goi truc tiep NhanVienBUS.add()
 *            va hien thi ket qua ngay trong dialog.
 *
 * Su dung:
 *   ThemNhanVienDialog dlg = new ThemNhanVienDialog(parentFrame);
 *   dlg.setVisible(true);
 *   if (dlg.isThemThanhCong()) { panel.loadDanhSach(); }
 */
public class ThemNhanVien extends JDialog {
    ...
    // ── Error labels ─────────────────────────────────────────
    private JLabel errMaNV, errHoten, errSdt, errTendangnhap, errMatkhau, errGlobal;

    // =========================================================
    // Constructor
    // =========================================================
    public ThemNhanVien(Frame parent) {
    ...
    private JPanel buildHeader() {
        ...
        JLabel lblSub = new JLabel("Điền đầy đủ thông tin để tạo tài khoản nhân viên mới");
    ...
    private JScrollPane buildForm() {
        ...
        form.add(makeRowDouble(
                makeFieldPanel("Mật khẩu *",       fMatkhau,   errMatkhau),
        ...
        // Label loi tong quat
        errGlobal = new JLabel(" ");
        errGlobal.setFont(new Font("Dialog", Font.BOLD, 12));
        errGlobal.setForeground(ERR_CLR);
        errGlobal.setAlignmentX(Component.LEFT_ALIGNMENT);
        form.add(errGlobal);
    ...
    private JPanel buildFooter() {
        ...
        JButton btnThem = makeBtn("Thêm mới", ACCENT, Color.WHITE);
    ...
    // =========================================================
    // Xu ly them moi — goi BUS truc tiep
    // =========================================================
    private void onThem() {
        System.out.println(">>> Bat dau xu ly bam nut Them"); // Dong nay de debug
        if (!doValidate()) {
            System.out.println(">>> Validation THAT BAI");
            return;
        }
        ...
        NhanVien nv = new NhanVien(maNV, hoten, sdt, chucvu, tendangnhap, matkhau, trangthai);

        boolean ok = bus.add(nv);
        if (ok) {
            themThanhCong = true;
            JOptionPane.showMessageDialog(this,
                    "Thêm nhân viên " + maNV + " — " + hoten + " thành công!",
                    "Thành công", JOptionPane.INFORMATION_MESSAGE);
            dispose();
        } else {
            errGlobal.setText("⚠  Thêm thất bại! Mã nhân viên hoặc tên đăng nhập đã tồn tại.");
        }
    }
    ...
    private void clearErrors() {
        ...
        errMatkhau.setText(" ");     errGlobal.setText(" ");
    }
    ...
    // Getter
    // =========================================================
    /** @return true neu da them thanh cong, false neu huy hoac that bai */
    public boolean isThemThanhCong() { return themThanhCong; }
